...
useEffect(() => {
  ...
  // 🆕 Зареждане от последния скан от localStorage
  try {
    const cached = localStorage.getItem("cg_last_email_scan");

    if (cached) {
      const parsed = JSON.parse(cached);

      // Последни резултати от скан
      if (parsed.emails) setEmails(parsed.emails);

      // Последен избран акаунт
      if (parsed.accountId) setSelectedAccountId(parsed.accountId);

      // Последна избрана папка
    ...
    <ProtectedRoute> 
      ...
      >
        ...
        <div className="section">
          ...
          {/* No Accounts Warning / Main Content */}
